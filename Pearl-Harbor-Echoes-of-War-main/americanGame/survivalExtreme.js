...
function animate(currentTime = 0) {
	...
	if (game.active && !game.over) {
		...
		timerEl.innerText = `Time: ${elapsedTime.toFixed(1)}s`
	}
	// clear / background
	c.fillStyle = '#44A7C4'
	...
	invaderProjectiles.forEach ((invaderProjectile, index) => {
		...
		if (
		  ...
		  setTimeout(() => {
		    ...
		    // Delay showing game over for explosion effect
		    setTimeout(() => {
		      const gameOverEl = document.getElementById('gameOver')
		      gameOverEl.style.display = 'block'
		      gameOverEl.innerHTML = `
		        <h1>Game Over</h1>
		        <p>You survived ${finalTime} seconds!</p>
		        <button id="restartButton" style=" font-size: 24px; padding: 12px 24px; cursor: pointer; background: #222; border: 2px solid white; color: white; border-radius: 6px;">Continue</button>
		      `
		      // Reattach restart button event
		      document.getElementById('restartButton').addEventListener('click', () => {
		        window.location.href = "./virgina.html";
		      })
		    }, 2000)
		  }, 0)
		}
	...
	if (game.active) {
		...
		// spawn new grids
		const now = Date.now();
		if (now - lastSpawnTime > spawnInterval / difficultyMultiplier) {
		  grids.push(new Grid());
		  lastSpawnTime = now;
		}
		...
		if (difficultyTimer % 200 === 0) {
			...
			spawnRate = Math.max(100, spawnRate - 100) // faster spawn rate
...
document.getElementById('startBtn').addEventListener('click', () => {
	...
	game.active = true
	startTime = Date.now() // start counting now
	elapsedTime = 0
})
